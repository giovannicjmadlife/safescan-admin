...
export async function PATCH(request: Request) {
  try {
    ...
    if (!id) {
      return NextResponse.json(
        { sucesso: false, erro: "ID do equipamento não informado." },
        { status: 400 }
      );
    }
    ...
    if (!["INATIVAR", "REATIVAR", "EXCLUIR"].includes(acao)) {
      return NextResponse.json(
        { sucesso: false, erro: "Ação inválida. Use INATIVAR, REATIVAR ou EXCLUIR." },
        { status: 400 }
      );
    }
    ...
    if (acao === "EXCLUIR") {
      ...
      if (error) {
        return NextResponse.json(
          { sucesso: false, erro: `Erro ao excluir equipamento: ${error.message}` },
          { status: 400 }
        );
      }

      return NextResponse.json({ sucesso: true, mensagem: "Equipamento excluído do catálogo." });
    }

    const novoStatus = acao === "REATIVAR" ? "ATIVO" : "INATIVO";
    ...
    if (error) {
      return NextResponse.json(
        { sucesso: false, erro: `Erro ao atualizar equipamento: ${error.message}` },
        { status: 400 }
      );
    }
    ...
    return NextResponse.json(
      { sucesso: false, erro: detalhe || "Erro interno ao atualizar catálogo." },
      { status: 500 }
    );
  }
}

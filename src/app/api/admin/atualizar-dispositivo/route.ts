...
export async function PATCH(request: NextRequest) {
  ...
  if (!id) {
    return criarRespostaErro("ID do dispositivo não informado.", 400);
  }

  if (!["PENDENTE", "APROVADO", "BLOQUEADO"].includes(status)) {
    return criarRespostaErro("Status inválido.", 400);
  }
  ...
  if (status === "APROVADO") {
    atualizacao.aprovado_por = admin.usuarioId;
    atualizacao.aprovado_em = agora;
    atualizacao.bloqueado_em = null;
  }

  if (status === "BLOQUEADO") {
    atualizacao.bloqueado_em = agora;
  }

  if (status === "PENDENTE") {
    atualizacao.aprovado_por = null;
    atualizacao.aprovado_em = null;
    atualizacao.bloqueado_em = null;
  }
  ...
  if (error) {
    return criarRespostaErro(
      `Erro ao atualizar dispositivo: ${error.message}`,
      500
    );
  }
